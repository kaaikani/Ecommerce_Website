...
import { getChannelsByCustomerPhonenumber, resendPhoneOtp, sendPhoneOtp } from "~/providers/customPlugins/customPlugin"
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!phoneNumber || phoneNumber.length !== 10) {
    return json({ message: "Invalid phone number", success: false }, { status: 400 })
  }
    ...
    try {
      ...
      try {
        ...
        if (channels && channels.length > 0) {
          ...
          return json({ sent, success: true, message: "OTP sent successfully" })
        } else {
...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("loginSuccess") === "true") {
      showNotification("success", "Login Successful", "You have been successfully logged in.")
    }
  }, [location])
    ...
    if (sendOtpFetcher.data) {
      ...
      if (data.sent) {
        ...
        showNotification(
          "error",
          "Not Registered",
          data.message || "This phone number is not registered. Please sign up first.",
        )
  ...
  useEffect(() => {
    if (loginFetcher.data) {
      ...
      if (data.userNotRegistered) {
        ...
        showNotification("error", "Login Failed", data.message || "Invalid OTP. Please try again.")
      }
    }
  }, [loginFetcher.data])
    ...
    <div className="min-h-screen flex flex-col lg:grid lg:grid-cols-3 rounded-xl">
      ...
      <div className="lg:hidden absolute bottom-0 left-0 right-0 z-10 bg-white rounded-t-3xl shadow-xl p-6 sm:p-8">
        <div className="mt-8 space-y-6 max-w-sm mx-auto">
          ...
          {!otpSent ? (
            <sendOtpFetcher.Form method="post" className="space-y-6" onSubmit={() => setHasSubmitted(true)}>
              ...
              <Button
                type="submit"
                disabled={!isClient || phoneNumber.length !== 10 || sendOtpFetcher.state !== "idle"}
            ...
            <loginFetcher.Form method="post" className="space-y-6">
              ...
              <p className="mt-10 text-center text-gray-500">
                ...
                <button
                  type="button"
                  onClick={() => resendOtpFetcher.submit({ phoneNumber, actionType: "resend-otp" }, { method: "post" })}
              ...
              <loginFetcher.Form method="post" className="space-y-6">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="h-4 w-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                  />
                  <label htmlFor="rememberMe-desktop" className="ml-2 text-sm text-gray-900">
                    Remember Me
                  </label>
                </div>
                ...
                <p className="mt-10 text-center text-gray-500">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      resendOtpFetcher.submit({ phoneNumber, actionType: "resend-otp" }, { method: "post" })
                    }
                    disabled={resendOtpFetcher.state !== "idle"}
                    className="font-semibold text-md text-indigo-600 hover:text-indigo-500"
                  >
