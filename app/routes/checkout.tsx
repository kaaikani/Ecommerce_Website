...
import { Form, useLoaderData, useOutletContext, useFetcher, useNavigate } from "@remix-run/react"
...
export async function loader({ request }: DataFunctionArgs) {
  ...
  if (activeOrder?.couponCodes && activeOrder.couponCodes.length > 0) {
    ...
    const appliedCouponDetails = couponCodes.find((c: any) => c.couponCode === appliedCouponCode)

    if (appliedCouponDetails) {
      const couponProductVariantIds: string[] = []
      for (const condition of appliedCouponDetails.conditions) {
        const variantArg = condition.args.find((arg: any) => arg.name === "productVariantIds")
        if (variantArg && variantArg.value) {
          try {
            ...
            if (Array.isArray(parsedIds)) {
              couponProductVariantIds.push(...parsedIds.map((id) => id.toString()))
      ...
      if (!hasCouponProducts && hasNonCouponProducts && activeOrder.lines.length > 0) {
        console.log("No coupon products remaining in cart, removing coupon:", appliedCouponCode)
        try {
...
export async function action({ request }: DataFunctionArgs) {
  ...
  if (action === "applyCoupon") {
    ...
    if (!coupon || !coupon.couponCode) {
      ...
      return json({ success: false, error: "Invalid coupon code." }, { status: 400 })
    }
    ...
    if (activeOrder?.couponCodes && activeOrder.couponCodes.length > 0) {
      ...
      return json(
        {
          success: false,
          error: "Another coupon is already applied. Please remove it before applying a new one.",
        },
        { status: 400 },
      )
    }
    ...
    const minAmountCondition = coupon.conditions.find(
      (c: any) => c.code === "minimum_order_amount" || c.code === "minimumOrderAmount" || c.code === "minimumAmount",
    )

    if (minAmountCondition) {
      const amountArg = minAmountCondition.args.find((a: any) => a.name === "amount") ?? minAmountCondition.args[0]
    ...
    if (couponResult?.__typename !== "Order") {
      ...
      return json({ success: false, error: "Unexpected response from server." }, { status: 500 })
    }
    ...
    return json({
      ...
      message: hasProductVariant
        ? `Coupon "${couponCode}" applied and ${productVariantIds.length} product${
  ...
  if (action === "removeCoupon") {
    ...
    if (!activeOrder?.couponCodes?.includes(couponCode)) {
      ...
      return json({ success: false, error: "Coupon code is not applied to the order." }, { status: 400 })
    }
    ...
    if (result?.__typename === "Order") {
      ...
      return json({ success: false, error: "Failed to remove coupon." }, { status: 400 })
    }
  }
...
export default function CheckoutPage() {
  ...
  const [shouldRefreshAfterCouponRemoval, setShouldRefreshAfterCouponRemoval] = useState(false)
  ...
  const isShippingMethodSelected = !!activeOrder?.shippingLines?.[0]?.shippingMethod
  ...
  const couponProductVariantIds = (appliedCouponDetails?.conditions || []).flatMap((condition: any) => {
  ...
  function handleRemoveItem(lineId: string) {
    ...
    const isCouponProduct = line && couponProductVariantIds.map(String).includes(String(line.productVariant.id))

    console.log("Removing line:", lineId, "isCouponProduct:", isCouponProduct, "appliedCouponCode:", appliedCouponCode)
  ...
  useEffect(() => {
    if (couponFetcher.data?.success && couponFetcher.data?.appliedCoupon === null && shouldRefreshAfterCouponRemoval) {
      ...
      <div className="lg:max-w-7xl max-w-2xl mx-auto pt-8 pb-24 px-4 sm:px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16">
          <div>
                ...
                <div className="mt-4 bg-white border rounded-lg shadow-sm p-4">
                      ...
                      <>
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="text-sm font-medium text-black">
                            {defaultShippingAddress ? "Default Shipping Address" : "Saved Address"}
                          </h3>
                          <Link to="/account/addresses" className="text-sm font-medium text-black">
                            Edit
                          </Link>
                        </div>
                  ...
                  {addressFormChanged && (
                    <div className="mt-6 pt-6 border-t border-black">
                      <h3 className="text-sm font-medium text-black mb-4">Select Another Address</h3>
            ...
            {activeOrder?.id && (
              <OrderInstructions orderId={activeOrder.id} initialValue={orderInstructions} disabled={false} />
            )}
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              ...
              {allLines.length > 3 && !showAllCartItems && (
                <div className="flex justify-center mt-2">
                  ...
                  >
                    <img src="/show-more.png" alt="Show more" className="w-6 h-6" />
                  </button>
                </div>
              )}
              {allLines.length > 3 && showAllCartItems && (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-600">Coupon "{appliedCouponCode}" applied</p>
                      </div>
                    </div>
                    <couponFetcher.Form method="post">
                      ...
                      >
                        {couponFetcher.state === "submitting" ? "Removing..." : "Remove"}
                      </button>
                    </couponFetcher.Form>
                  </div>
                </div>
