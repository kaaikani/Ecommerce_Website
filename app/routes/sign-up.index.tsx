...
import { Link, useActionData, useLoaderData, useNavigation, useFetcher, useLocation } from "@remix-run/react"
import { type ActionFunctionArgs, json, type LoaderFunctionArgs, redirect } from "@remix-run/server-runtime"
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "send-otp" && phoneNumber) {
    try {
      ...
      try {
        const existingChannels = await getChannelsByCustomerPhonenumber(phoneNumber)
  ...
  if (intent === "submit-form") {
    ...
    if (!selectedChannelToken) {
      return json(
        { success: false, message: "No channel was selected. Please try again." },
        {
          status: 400,
        },
      )
...
export default function SignUpPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("SignupSuccess") === "true") {
      showNotification("success", "Signup Successful", "You have been successfully registered and logged in.")
    }
  }, [location])
    ...
    if (sendOtpFetcher.data) {
      ...
      if (data.userAlreadyRegistered) {
        ...
        showNotification(
          "error",
          "Already Registered",
          data.message || `This phone number is already registered. Please sign in instead.`,
        )
      } else if (data.sent) {
        // OTP sent successfully
        setOtpSent(true)
        showNotification("success", "OTP Sent", data.message || "OTP has been sent to your phone.")
      } else if (data.success === false) {
        // Other error
        showNotification("error", "Error", data.message || "Failed to send OTP.")
      }
    }
  }, [sendOtpFetcher.data])
  ...
  useEffect(() => {
    if (signUpFetcher.data) {
      const data = signUpFetcher.data as { success?: boolean; message?: string }
      if (data.success === false) {
        showNotification("error", "Signup Failed", data.message || "Invalid OTP. Please try again.")
      }
    }
  }, [signUpFetcher.data])
  ...
  useEffect(() => {
    if (resendOtpFetcher.data) {
      const data = resendOtpFetcher.data as { sent?: boolean; success?: boolean; message?: string }
      if (data.sent) {
        showNotification("success", "OTP Resent", data.message || "OTP has been resent to your phone.")
      } else if (data.success === false) {
    ...
    <div className="min-h-screen flex flex-col lg:grid lg:grid-cols-3 rounded-xl">
      ...
      <div className="lg:hidden absolute bottom-0 left-0 right-0 z-10 bg-white rounded-t-3xl shadow-xl p-6 sm:p-8">
        <div className="mt-8 space-y-6 max-w-sm mx-auto">
          ...
          {!otpSent ? (
            <SendOtpForm method="post" className="space-y-6" onSubmit={() => setHasSubmitted(true)}>
              ...
              <Button
                type="submit"
                disabled={!isClient || formData.phoneNumber.length !== 10 || sendOtpFetcher.state !== "idle"}
            ...
            <signUpFetcher.Form method="post" className="space-y-6">
              ...
              <p className="mt-10 text-center text-gray-500">
                ...
                <button
                  ...
                  onClick={() =>
                    resendOtpFetcher.submit(
                      { phoneNumber: formData.phoneNumber, intent: "resend-otp" },
                      { method: "post" },
                    )
                  }
                ...
                >
                  {resendOtpFetcher.state !== "idle" ? "Resending..." : "Resend OTP"}
                </button>
              </p>
            </signUpFetcher.Form>
          )}
          ...
          <div className="mt-8 space-y-6">
            {!otpSent ? (
              <SendOtpForm method="post" className="space-y-6" onSubmit={() => setHasSubmitted(true)}>
                ...
                <div className="space-y-2">
                  <label htmlFor="phoneNumber-desktop" className="block text-sm font-medium text-gray-700">
                    Phone Number
                  </label>
                  <input
              ...
              <signUpFetcher.Form method="post" className="space-y-6">
                ...
                <div>
                  <label htmlFor="otp-desktop" className="block text-sm font-medium text-gray-700">
                    OTP is Sent to Your Registered Phone Number
                  </label>
                  <input
                    id="otp-desktop"
      ...
      <div className="hidden lg:flex lg:col-span-2 items-center justify-center relative p-12 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl p-8">
          <img src="/banner.jpg" alt="Product" className="mx-auto w-full rounded-lg shadow-md" />
        </div>
      </div>
    </div>
  )
