...
// Lucide icons
import { MapPin, ShoppingBag, User, Menu, X, LogOut, Check, Edit, Mail, Phone } from "lucide-react"
...
export async function action({ request }: DataFunctionArgs) {
  ...
  if (intent === FormIntent.UpdateEmail) {
    ...
    if (updateResult.__typename !== "Success") {
      return formError({ message: updateResult.message, intent: FormIntent.UpdateEmail }, { status: 401 })
    }
  ...
  if (intent === FormIntent.UpdateDetails) {
    ...
    await updateCustomer({ title, firstName, lastName, phoneNumber: activeCustomer.phoneNumber }, { request })
        ...
        <DialogContent className="sm:max-w-md">
          <ValidatedForm validator={changeEmailValidator} method="post">
            <DialogHeader>
              ...
              <DialogDescription>{t("account.changeEmailModal.heading")}</DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Label htmlFor="email">{t("account.changeEmailModal.new")}</Label>
                <Input
                  id="email"
                  name="email"
                  type="email"
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={closeChangeEmailModal}>
                {t("common.cancel")}
              </Button>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <Input id="firstName" name="firstName" required className="mt-1" defaultValue={firstName || ""} />
                    </div>

                    <div>
                      <Label htmlFor="lastName">Last Name *</Label>
                      <Input id="lastName" name="lastName" required className="mt-1" defaultValue={lastName || ""} />
                    </div>
                  </div>

                  {formError && formError.intent === FormIntent.UpdateDetails && (
                    <div className="mt-4">
                      <ErrorMessage heading="Error" message={formError.message} />
                    </div>
                  )}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-sm mt-1">{replaceEmptyString(fullName)}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Phone Number</Label>
                    <p className="text-sm mt-1">{replaceEmptyString(phoneNumber) || "+1 (555) 123-4567"}</p>
                  </div>
                </div>
