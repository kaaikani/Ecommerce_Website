...
export function Logo(props: React.ComponentPropsWithoutRef<'img'>) {
  return (
    <img
      src="/KK-Logo.png" // <-- update path based on your project structure
      alt="Your Company Logo"
      className="h-6" // Tailwind styling, tweak as needed
      {...props}
    />
  )
}
...
>(function AppScreenHeader({ children }, ref) {
  return (
    <div ref={ref} className="mt-6 px-4 bg-gray-300 text-white">
      {children}
    </div>
  )
})
...
>(function AppScreenTitle({ children }, ref) {
  return (
    <div ref={ref} className="text-2xl bg-gray-300 text-white">
      {children}
    </div>
  )
})
...
>(function AppScreenSubtitle({ children }, ref) {
  return (
    <div ref={ref} className="text-sm bg-gray-300 text-gray-500">
      {children}
    </div>
  )
})
...
>(function AppScreenBody({ children, className }, ref) {
  return (
    <div
      ref={ref}
      className={clsx('mt-6 flex-auto rounded-t-2xl bg-green-100', className)}
    >
      {children}
    </div>
  )
