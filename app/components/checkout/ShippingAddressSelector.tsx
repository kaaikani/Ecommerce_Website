        ...
        {addresses.map((address, index) => (
          <RadioGroup.Option
            ...
            className={({ checked, active }) =>
              classNames(
                checked
                  ? 'border-primary-500 ring-2 ring-black'
                  : 'border-gray-300',
                'relative bg-white border rounded shadow-sm p-4 flex items-start cursor-pointer focus:outline-none transition',
              )
            }
          >
            {({ checked }) => (
              <>
                <div className="flex-1">
                  ...
                  <RadioGroup.Description
                    as="div"
                    className="text-sm text-black mt-1 space-y-0.5"
                  >
                    <p>
                      {address.streetLine1}
                      {address.streetLine2
                ...
                {checked && (
                  <CheckCircleIcon
                    className="h-5 w-5 text-black ml-2"
                    aria-hidden="true"
                  />
                )}
                <span
                  className={classNames(
                    ...
                    'absolute -inset-px rounded pointer-events-none border-2',
                  )}
                  aria-hidden="true"
                />
              </>
