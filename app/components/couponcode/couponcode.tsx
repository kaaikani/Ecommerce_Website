...
import { getCouponCodeList, applyCouponCode, removeCouponCode } from '~/providers/orders/order';
  ...
  if (actionType === 'apply') {
    ...
    try {
      ...
      if (result?.__typename === 'Order') {
        ...
        return json({ error: result.message || 'Invalid coupon code.' }, { status: 400 });
      }
  ...
  return json({ error: 'Invalid action type.' }, { status: 400 });
};export default function CouponsComponent({ couponCodes, appliedCoupon }: CouponsComponentProps) {
    ...
    return (
      <div className="mt-4 bg-black w-80 p-3 rounded-md shadow border border-gray-200 text-sm">
        <h3 className="text-base font-semibold text-gray-800 mb-3">Coupon Codes</h3>
