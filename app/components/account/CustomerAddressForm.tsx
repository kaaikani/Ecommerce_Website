...
const AddressTypeSelect = () => {
  ...
  return (
    <LabelInputContainer>
      <label htmlFor="addressType" className="text-sm mt-5 font-medium text-neutral-800">
...
export const PostalCodeSelect = ({
  ...
  const matchingPostalcode = channelPostalcodes.find((p) => p.cityName === channelCode);
  const postalCodes = matchingPostalcode?.code.map((code) => code.toString()) || [];
  ...
  return (
    <div className="w-full">
      ...
      <Select value={value} onValueChange={handleChange} disabled={postalCodes.length === 0}>
        <SelectTrigger className="mt-1 w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 enabled:cursor-pointer enabled:hover:border-cyan-500 disabled:bg-neutral-100 disabled:text-neutral-500 disabled:border-neutral-400">
          <SelectValue placeholder={postalCodes.length === 0 ? "No pincodes available" : "Select Pincode"} />
        </SelectTrigger>
...
export default function CustomerAddressForm({
  ...
  submit: () => void;
  isEditing?: boolean;
  activeCustomer?: { firstName?: string; lastName?: string; phoneNumber?: string };
  channelCode: string;
  channelPostalcodes: ChannelPostalcode[];
  ...
  return (
    <div className="mx-auto w-full max-w-4xl bg-white" style={{ maxHeight: "90vh" }}>
      <div className="flex-1 overflow-y-auto px-6 py-4">
        ...
        <ValidatedForm
          ...
          defaultValues={{
            ...
            phone: isEditing ? address?.phoneNumber || "" : activeCustomer?.phoneNumber || "",
            company: address?.company || "",
            addressType: isEditing && address
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <ModernInput
              ...
              type="tel"
              autoComplete="tel"
              defaultValue={isEditing ? address?.phoneNumber || "" : activeCustomer?.phoneNumber || ""}
            />
          </div>
