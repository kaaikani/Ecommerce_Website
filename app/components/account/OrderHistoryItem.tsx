...
export default function OrderHistoryItem({
  ...
  const canCancelOrder = order?.state && !["Cancelled", "Delivered", "Shipped"].includes(order.state)

  // Check if cancel request is already pending
  const hasPendingCancelRequest = order?.customFields?.clientRequestToCancel || false;
      ...
      <div className="p-4 lg:p-6 flex flex-row justify-between items-center bg-gray-50 border-b">
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-8 lg:gap-16 text-sm">
          ...
          <div>
            ...
            <span
              ...
              title={order.orderPlacedAt ? new Date(order.orderPlacedAt).toLocaleString() : ""}
            >
              {order.orderPlacedAt
        ...
        <div className="gap-4 lg:gap-6 flex flex-col items-end self-stretch justify-between md:flex-row md:items-center">
          <div className="flex flex-col items-end gap-2">
            ...
            <span className="text-xs text-orange-600 bg-orange-50 px-2 py-1 rounded">Cancel request pending</span>
          )}
          </div>
          <div className="flex" role="group">
            {canCancelOrder && !hasPendingCancelRequest && (
              <Button
                ...
                className="bg-red-50 text-red-600 hover:bg-red-100 text-sm rounded-r-none border-r-0"
                title="Cancel Order"
      ...
      {isExpanded && (
        <div className="flex flex-col">
          {order.lines.map((line, key) => (
            <div key={line.id || key} className="p-4 lg:p-6 border-b flex flex-row gap-8 justify-between group">
              {/* Product */}
              <div className="inline-flex justify-center items-center gap-4">
                ...
                >
                  <img
                    src={line.featuredAsset?.source || "/placeholder.svg?height=96&width=96"}
                    ...
                    {line.fulfillmentLines
                      ...
                      .map((fLine, index) => (
                        <span
                          ...
                          title={
                            fLine.fulfillment.updatedAt ? new Date(fLine.fulfillment.updatedAt).toLocaleString() : ""
                          }
                        >
          ...
          <div className="p-2 lg:py-3 lg:px-6 gap-2 lg:gap-6 grid grid-cols-2 sm:flex justify-end items-center">
            {order.fulfillments?.map((fulfillment, index) => (
              <Button
                ...
                onClick={() => alert(`${t("trackAlert")} "${fulfillment.trackingCode || "N/A"}"`)}
                className="text-xs"
              >
                {/* Only show package number if there are more than one: Looks cleaner */}
                {t("order.trackPackage")} {(order.fulfillments?.length || 0) === 1 ? "" : `#${index + 1}`}
              </Button>
            ))}
             {hasPendingCancelRequest && order.state !== "Cancelled" && (
          ...
          {areDetailsExpanded && (
            <div className="p-2 lg:p-3 grid grid-cols-2 gap-1 text-sm max-w-sm self-center md:self-end">
              ...
              <span className="text-end">
                <Price  priceWithTax={order.subTotalWithTax} />
              </span>

              <span>{t("order.shippingAndHandling")}</span>
              <span className="text-end">
                <Price
                  priceWithTax={order.shippingLines.reduce((acc, shippingLine) => acc + shippingLine.priceWithTax, 0)}
                />
              </span>
              ...
              <span className="text-end">
                <Price
                  priceWithTax={order.taxSummary?.reduce((acc, tax) => acc + tax.taxBase, 0) || 0}
                />
              </span>
              ...
              <span className="text-end">
                <Price
                  priceWithTax={order.taxSummary?.reduce((acc, tax) => acc + tax.taxTotal, 0) || 0}
                />
              </span>
              ...
              <span className="text-end">
                <Price
                  priceWithTax={
                    order.totalWithTax - order.discounts.reduce((acc, discount) => acc + discount.amountWithTax, 0)
                  }
                />
              </span>
              ...
              <span className="text-end">
                <Price
                  priceWithTax={order.discounts.reduce((acc, discount) => acc + discount.amountWithTax, 0)}
                />
              </span>
